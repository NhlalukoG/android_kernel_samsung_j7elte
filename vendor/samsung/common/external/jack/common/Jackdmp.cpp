...
static void jackctl_server_switch_master_dummy(jackctl_server_t * server_ctl, char * master_driver_name)
{
    ...
    if(is_dummy_driver) {
        ...
        jackctl_driver_update_parameters(master_driver_name, driver_ctr);
    } else {
...
int main(int argc, char** argv)
{
    ...
    // Waits for signal
#ifdef __ANDROID__
    jackctl_driver_create_backup_parameters(master_driver_ctl);
    ...
    while(1) {
        ...
        if (signal == SIGUSR2) {
            ...
            jackctl_send_command_to_jack_logger(1);

        } else if(signal == SIGUSR2 + 2 ){
            jackctl_send_command_to_jack_logger(2);
