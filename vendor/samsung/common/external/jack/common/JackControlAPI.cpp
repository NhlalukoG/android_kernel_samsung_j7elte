    ...
    JSList ** retparamlist)
{
    ...
    *retparamlist = NULL;
    int jack_param_count = 0;
    while (paramlist != NULL)
    {
        param_ptr = (jackctl_parameter *)paramlist->data;
        if(jack_param_count++ > 100){
            jack_info("jackctl_create_param_list [%d] is_set %d, type %d", jack_param_count, param_ptr->is_set, param_ptr->type);
        }
...
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
static
JackLogger* jackctl_get_jack_logger(){
    static JackLogger * jack_logger = NULL;
    
    if(jack_logger == NULL){
        typedef void * (*fJackLogger)();
        void* tLoggerLib = NULL;
        jack_log("jackctl_get_jack_logger");
        dlerror();
        ...
        if(tLoggerLib == NULL) {
            ...
            iJackLoggerInstance = (fJackLogger) dlsym(tLoggerLib, "Instance");
             if(iJackLoggerInstance != NULL){
                jack_logger = (JackLogger *) iJackLoggerInstance();
            }else{
...
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
    sigaddset(&sigmask.signals, SIGUSR2 + 1);
    sigaddset(&sigmask.signals, SIGUSR2 + 2);
#endif
    ...
    jackctl_driver *driver_ptr)
{
    ...
    try {
        ...
        jack_info("done JackServer open");
        return true;
...
fail_delete:
    jack_error("jack_server_open fail_delete");
    delete server_ptr->engine;
    server_ptr->engine = NULL;
...
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
SERVER_EXPORT int
jackctl_send_command_to_jack_logger(int cmd)
{
    static bool isJackPcmDump = false;
    JackLogger * pJackLogger = jackctl_get_jack_logger();
    if(pJackLogger == NULL){
        jack_error("can't get jack logger instance");
        return -1;
    }
    switch(cmd){
        case 1:
            jack_log("jackctl_send_command_to_jack_logger, isJackPcmDump!! %d", isJackPcmDump);            
            if(isJackPcmDump == false){
                pJackLogger->excuteCommand(JackLogger::LOG_COMMAND_PCM_DUMP, JackLogger::COMMAND_PARAM_PCM_DUMP_START);
                isJackPcmDump = true;
            }else{
                pJackLogger->excuteCommand(JackLogger::LOG_COMMAND_PCM_DUMP, JackLogger::COMMAND_PARAM_PCM_DUMP_STOP);
                isJackPcmDump = false;
            }
            break;
        case 2:
            pJackLogger->dumpLog(0, NULL, JackLogger::LOG_BASE_JACKD);
            break;
        default:
            jack_error("jackctl_send_command_to_jack_logger invalid cmd %d", cmd);
            break;
    }
    return 0;
}
